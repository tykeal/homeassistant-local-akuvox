...
def test_capability_report_dependency_floor() -> None:
    """Test dependency metadata requires pylocal-akuvox 1.5.0 or newer."""
    manifest = json.loads(
    ...
    assert "pylocal-akuvox>=1.5.0" in manifest["requirements"]
    assert "pylocal-akuvox>=1.5.0" in pyproject["project"]["dependencies"]
    assert Version(lock_package["version"]) >= Version("1.5.0")
    assert Version(importlib.metadata.version("pylocal-akuvox")) >= Version("1.5.0")
