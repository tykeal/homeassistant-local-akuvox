...
- [X] T001 [US1] Re-confirm upstream v1.0.0 symbols against the checked-out
  ...
  SC-007.
- [X] T002 [US1] Inventory every current dependency pin and stale version comment
...
- [X] T003 [US1] Run `uv run pytest tests/ -q` from the repository root before
  edits and record any pre-existing failures in the implementation PR notes.
  This guards FR-021, FR-022, and SC-008.
- [X] T004 [P] [US5] Run `uv run ruff check custom_components/ tests/` and
...
- [X] T005 [P] [US1] Update `tests/conftest.py` so the shared `AkuvoxDevice`
...
- [X] T006 [P] [US1] Add failing tests in `tests/test_create_device.py` for an
...
- [X] T007 [P] [US1] Add failing repairs-helper tests in a new
...
- [X] T008 [P] [US2] Add failing translation/schema tests in
...
- [X] T009 [US1] Raise the dependency floor to `pylocal-akuvox>=1.0.0` in
...
- [X] T010 [US1] Run `uv sync` or the repository's equivalent `uv lock` workflow
...
- [X] T011 [US2] Add `CONF_ATTEMPT_UNKNOWN_CAPABILITY =
...
- [X] T012 [US2] Add setup-flow and options-flow strings for
...
- [X] T013 [US1] Create `custom_components/local_akuvox/capability_support.py`
...
- [X] T014 [P] [US1] Update imports and typing in tests for `Capability`,
...
- [X] T015 [P] [US1] Add failing tests in `tests/test_init.py` for
...
- [X] T016 [P] [US1] Add failing tests in `tests/test_config_flow.py` for
  ...
  SC-002, and SC-008.
- [X] T017 [P] [US1] Add failing tests in `tests/test_options_flow.py` and
...
- [X] T018 [P] [US1] Add failing coordinator tests in `tests/test_coordinator.py`
...
- [X] T019 [P] [US1] Add failing service-call tests in `tests/test_lock.py`,
...
- [X] T020 [US1] Update `custom_components/local_akuvox/__init__.py` so
...
- [X] T021 [US1] Update `custom_components/local_akuvox/config_flow.py` to wrap
...
- [X] T022 [US1] Update `custom_components/local_akuvox/options_flow.py` so
...
- [X] T023 [US1] Update `custom_components/local_akuvox/coordinator.py` to add
...
- [X] T024 [US1] Update `custom_components/local_akuvox/lock.py` service and
...
- [X] T025 [US1] Update `custom_components/local_akuvox/webhook.py` background
  ...
  SC-003, SC-005, and SC-008.
- [X] T026 [US1] Run targeted MVP tests with
...
- [X] T027 [P] [US2] Extend `tests/test_config_flow.py` with failing tests for a
...
- [X] T028 [P] [US2] Extend `tests/test_options_flow.py` with failing tests that
...
- [X] T029 [P] [US2] Extend `tests/test_lock.py` and `tests/test_services.py`
...
- [X] T030 [US2] Update `custom_components/local_akuvox/config_flow.py` to add a
...
- [X] T031 [US2] Update `custom_components/local_akuvox/options_flow.py` to add
  ...
  FR-013, and SC-004.
- [X] T032 [US2] Update `custom_components/local_akuvox/__init__.py`,
  ...
  SC-004.
- [X] T033 [US2] Run targeted opt-in tests with
...
- [X] T034 [P] [US3] Add failing `tests/test_lock.py` cases where
...
- [X] T035 [P] [US3] Add failing `tests/test_lock.py` cases for
...
- [X] T036 [US3] Update `custom_components/local_akuvox/lock.py` entity setup and
...
- [X] T037 [US3] Update `custom_components/local_akuvox/lock.py` lock/unlock
  ...
  SC-006.
- [X] T038 [US3] Run `uv run pytest tests/test_lock.py tests/test_coordinator.py -q`
...
- [X] T039 [P] [US4] Add failing tests in a new `tests/test_diagnostics.py` for
...
- [X] T040 [P] [US4] Add failing diagnostics tests proving the user-triggered
...
- [X] T041 [US4] Create `custom_components/local_akuvox/diagnostics.py` with SPDX
...
- [X] T042 [US4] Add shared serialization/sanitization support in
...
- [X] T043 [US4] Run `uv run pytest tests/test_diagnostics.py tests/test_capability_support.py -q`
...
- [X] T044 [P] [US5] Update release metadata for the implementation PR: because
...
- [X] T045 [US5] Run the full test suite with coverage:
  ...
  SC-008.
- [X] T046 [P] [US5] Run `uv run ruff check custom_components/ tests/` and fix
  all lint errors without changing intended behavior. Covers FR-022 and SC-008.
- [X] T047 [P] [US5] Run `uv run ruff format --check custom_components/ tests/`
  and fix formatting only if the check reports required changes. Covers FR-022
  and SC-008.
- [X] T048 [P] [US5] Run `uv run mypy custom_components tests` and fix all type
...
- [X] T050 [P] [US5] Run `npx --yes aislop@0.12.0 ci` and split helpers if any
...
- [X] T051 [US5] Run `uv run pre-commit run --all-files` before the
  ...
  SC-008.
- [X] T052 [US5] In the implementation PR, use a breaking conventional commit
