...
reference for this plan. Stage 5 should re-check the exact installed v1.1.0
...
the report data. The verified v1.1.0 source still redirects stdout/stderr for
...
v1.1.0 debug logging redacts the password but includes `UserName` in the
...
data, or saved JSON. Because upstream v1.1.0 debug logging can include
...
`pylocal-akuvox>=1.0.0` to `pylocal-akuvox>=1.1.0` in `manifest.json` and
...
the new `1.1.0` dependency floor. Review `.pre-commit-config.yaml` comments and
...
- Optional import with old-library fallback — rejected because the feature cannot
  work without the v1.1.0 API and would complicate tests and user errors.
- Exact `==1.1.0` pin — rejected because the repository uses minimum compatible
...
| Dependency floor | `pylocal-akuvox>=1.1.0` in runtime and project metadata |
