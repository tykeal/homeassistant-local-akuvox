...
reference for this plan. Stage 5 should re-check the exact installed v1.4.0
...
the report data. The verified v1.4.0 source still redirects stdout/stderr for
...
v1.4.0 debug logging redacts the password but includes `UserName` in the
...
data, or saved JSON. Because upstream v1.4.0 debug logging can include
...
`pylocal-akuvox>=1.0.0` to `pylocal-akuvox>=1.4.0` in `manifest.json` and
...
the new `1.4.0` dependency floor. Review `.pre-commit-config.yaml` comments and
...
- Exact `==1.4.0` pin — rejected because the repository uses minimum compatible
...
| Dependency floor | `pylocal-akuvox>=1.4.0` in runtime and project metadata |
