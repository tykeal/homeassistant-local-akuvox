...
**Purpose**: Re-confirm live source facts, then make the v1.4.0 report API
...
- [X] T002 [US1] Raise the `pylocal-akuvox` floor from `>=1.0.0` to
  `>=1.4.0` in `custom_components/local_akuvox/manifest.json`,
...
- [X] T003 [US1] Run `uv sync` from the repository root, commit the
  ...
  `uv lock --check` accepts `pylocal-akuvox>=1.4.0`. Covers FR-018 and
  SC-006.
- [X] T004 [P] [US1] Add dependency/import regression tests in the
  ...
  import all resolve `pylocal-akuvox>=1.4.0` and export
...
**Checkpoint**: The v1.4.0 API is importable and live-source facts are
...
- [X] T024 [US3] Ensure `custom_components/local_akuvox/services.py` and
  ...
  credentials only to upstream, and document the upstream v1.4.0 username
...
- [X] T034 [US1] Add matching service labels, field labels,
  descriptions, validation text, and pylocal-akuvox v1.4.0 capability text
  to `custom_components/local_akuvox/strings.json`. Covers FR-020,
  FR-021, FR-023, FR-027, SC-003, SC-004, and SC-006.
- [X] T035 [US1] Mirror the new service strings and v1.4.0 capability text
...
1. Complete the v1.4.0 pin bump and lock refresh.
