...
- The upstream write suite is broader than throwaway CRUD. In v1.3.0 it also
...
- **FR-015**: The service MUST pass a no-op `emit` callback when invoking
  ...
  v1.3.0 `run_capability_report()` performs process-wide stdout/stderr
...
- **FR-018**: The integration MUST raise the minimum `pylocal-akuvox` dependency
  pin from `>=1.0.0` to `>=1.3.0` in runtime and project metadata.
