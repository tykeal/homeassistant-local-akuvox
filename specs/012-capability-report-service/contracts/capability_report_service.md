...
| `open_door_user` | optional non-empty string | `None` | Required when `open_door=True`; passed as `open_door_user`; never logged by integration code, persisted, returned, saved, or placed in repairs/errors. Upstream v1.1.0 debug logs can include it unless an upstream fix is consumed. |
...
7. Acquire one Home Assistant instance-wide capability-report lock stored under a
   ...
   before calling upstream because the verified v1.1.0 source redirects
...
- Valid OpenDoor passes user/password to upstream, does not log either field from
  integration code, and documents the upstream v1.1.0 username debug-log caveat.
