...
| `open_door_user` | non-empty string | `None` | Required when `open_door=True` | Relay/OpenDoor HTTP username, separate from integration credentials. Never log from integration code, store, return, save, or include in repairs placeholders. Upstream v1.6.0 debug logs can include this username unless an upstream redaction fix is consumed. |
...
The upstream v1.6.0 `DiagnosticReport.to_json()` contract currently returns:
