...
`pylocal_akuvox.run_capability_report()` from `pylocal-akuvox>=1.6.0`, hard-gate
...
`pylocal-akuvox>=1.6.0`, voluptuous service schemas, pytest Home Assistant
...
.pre-commit-config.yaml             # Keep validation hooks; update comments or
                                     # dependency assumptions if v1.6.0 requires it
pyproject.toml                      # Bump pylocal-akuvox dependency to >=1.6.0
                                     # and update stale dependency comments
uv.lock                             # Refresh after dependency bump

custom_components/local_akuvox/
├── manifest.json                   # Bump runtime requirement to >=1.6.0
...
│                                    # update pylocal-akuvox version text to 1.6.0
├── translations/
│   └── en.json                     # Mirror user-facing service strings and the
│                                    # pylocal-akuvox 1.6.0 version text
