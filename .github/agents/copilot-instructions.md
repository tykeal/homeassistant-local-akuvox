...
## Active Technologies
- Python ≥3.14.2 with pylocal-akuvox (001-akuvox-lock-integration)
- Home Assistant config entries (001-akuvox-lock-integration)
- Python ≥3.14.2 with homeassistant, pylocal-akuvox ≥0.2.3, voluptuous (002-device-config-discovery)
- Python ≥3.14.2 + pylocal-akuvox ≥0.2.3 schedule/user services (003-schedule-user-services)
- Python ≥3.14.2 + pylocal-akuvox ≥0.2.3, HA webhook infra (004-webhook-endpoint)
- HA config entries with webhook_id, webhook_enabled (004-webhook-endpoint)
- Python ≥3.14.2 + pylocal-akuvox ≥0.2.3 device communication (005-add-lock-action)
- N/A storage, uses existing coordinator data and entity state (005-add-lock-action)
- Python ≥3.14.2 (type-annotated, mypy-checked) + homeassistant, voluptuous, pylocal-akuvox (008-service-layer-extraction)
...
Python ≥3.14.2: Follow standard HA integration conventions
...
- 004-webhook-endpoint: Planned (design phase) Python ≥3.14.2 +
